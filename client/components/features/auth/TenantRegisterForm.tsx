...
import { Loader2, Eye, EyeOff, User, Mail, Lock, Phone, FileText, IdCard, Upload, ChevronRight } from "lucide-react";
import { GoogleLogin } from "@react-oauth/google";
import Link from "next/link";
import { CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";

export function TenantRegisterForm({ onSwitchToLogin }: { onSwitchToLogin?: () => void }) {
  const router = useRouter();
  const dispatch = useDispatch();
  const [formData, setFormData] = useState({ 
    name: "", email: "", mobile: "", password: "", confirmPassword: "", docType: "Aadhaar", docId: "" 
  });
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await authApi.registerTenant(submitData);
      
setSuccess("Account created successfully! Switching to login...");      setTimeout(() => {
  ...
  return (
    <div className="space-y-5">
      <CardHeader className="px-0 text-center pt-0">
        <CardTitle className="text-2xl font-bold">Create a Seeker Account</CardTitle>
        <CardDescription>Register to find and book properties</CardDescription>
      </CardHeader>
      ...
      <CardContent className="px-0 space-y-5">
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          {/* Name and Mobile Row - More compact */}
          ...
          {/* Identity Verification - Compact & Styled */}
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-xs text-muted-foreground">
                  Upload clear scanned copy (PDF only, max 5MB)
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="text-center text-sm text-muted-foreground">
            ...
            {onSwitchToLogin ? (
              ...
              <Link href="/login?role=tenant" className="text-primary hover:underline font-medium">
                Sign in
              </Link>
            )}
          </div>
        ...
        {/* Google Button at Bottom with OR Option */}
